...
export function getAlarmName(reminderId) {
    return `reminder:${reminderId}`;
}
/** 產生 snooze alarm 名稱，格式: snooze:{id} */
export function getSnoozeAlarmName(reminderId) {
...
export function parseAlarmName(alarmName) {
    ...
    if (!match)
        return null;
    return { type: match[1], reminderId: match[2] };
}
// ============================================================
// 計算下一次觸發時間
// ============================================================
/** 計算單次提醒的觸發時間 */
export function getNextFireTimeOneTime(schedule) {
    const targetTime = new Date(schedule.dateTime).getTime();
    return targetTime > Date.now() ? targetTime : null;
}
/** 計算週期提醒的下一次觸發時間 */
export function getNextFireTimeRecurring(schedule) {
    const now = new Date();
...
// ============================================================
/** 為某筆提醒建立 alarm */
export async function createAlarmForReminder(reminder) {
    if (!reminder.enabled)
        return;
    // first_open 類型不使用 alarm，由 browser activity 觸發
    if (reminder.type === 'first_open')
        return;
...
/** 清除某筆提醒的 alarm */
export async function clearAlarmForReminder(reminderId) {
    await chrome.alarms.clear(getAlarmName(reminderId));
    await chrome.alarms.clear(getSnoozeAlarmName(reminderId));
}
/** 建立 snooze alarm */
export async function createSnoozeAlarm(reminderId, delayMinutes) {
...
// ============================================================
/** 為所有啟用中的提醒重建 alarm */
export async function rebuildAllAlarms(reminders) {
    // 先清除所有既有 alarm
