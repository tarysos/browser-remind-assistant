 ...
 */
function countPending(reminders) {
    const now = Date.now();
    let count = 0;
    for (const r of reminders) {
        ...
        if (r.type === 'one_time') {
            ...
            if (s.completedAt)
                continue;
            if (s.snoozeUntil && new Date(s.snoozeUntil).getTime() > now)
                continue;
            // 時間已到且未完成
            const targetTime = new Date(r.schedule.dateTime).getTime();
            if (targetTime <= now && !s.triggeredAt)
                count++;
            if (s.triggeredAt && !s.completedAt)
                count++;
        }
        else {
            const s = r.state;
            ...
            // completedAt 只阻擋當期
            if (isCompletedInCurrentPeriod(s.completedAt, freq))
                continue;
            if (s.snoozeUntil && new Date(s.snoozeUntil).getTime() > now)
                continue;
            // 已觸發但本 period 尚未完成
...
function countToday(reminders) {
    ...
    for (const r of reminders) {
        ...
        if (r.type === 'one_time') {
            const target = new Date(r.schedule.dateTime);
