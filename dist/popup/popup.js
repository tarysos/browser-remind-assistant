...
// ============================================================
import { getAllRemindersArray, getReminderById, updateReminder, addHistoryEntry, } from '../modules/reminder-repository/index.js';
...
const listSnoozed = document.getElementById('list-snoozed');
const emptyPending = document.getElementById('empty-pending');
const btnAdd = document.getElementById('btn-add');
const linkOptions = document.getElementById('link-options');
function categorize(reminders) {
    const now = new Date();
    const result = { pending: [], upcoming: [], snoozed: [] };
    for (const r of reminders) {
        ...
        if (r.type === 'one_time') {
            if (r.state.completedAt)
                continue;
        }
        else {
            const completedAt = r.state.completedAt;
            if (isCompletedInCurrentPeriod(completedAt, getReminderFrequency(r)))
                continue;
        }
        // 判斷是否為今日待處理
        if (r.type === 'one_time') {
            const targetTime = new Date(r.schedule.dateTime);
    ...
    const getSortTime = (r) => {
        if (r.type === 'one_time')
            return new Date(r.schedule.dateTime).getTime();
        if (r.type === 'recurring') {
            const [h, m] = r.schedule.timeOfDay.split(':').map(Number);
            return h * 60 + m;
        }
...
// ============================================================
/** 取得 snoozeUntil 值 */
function getSnoozeUntil(r) {
    return r.type === 'one_time'
        ? r.state.snoozeUntil
        : r.state.snoozeUntil;
}
/** 判斷是否正在 snooze 中 */
function isSnoozed(r) {
...
function renderReminderItem(r) {
    ...
    const timeStr = r.type === 'one_time'
        ? new Date(r.schedule.dateTime).toLocaleString('zh-TW', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' })
        : r.type === 'recurring'
            ? `${r.schedule.frequency === 'daily' ? '每天' : r.schedule.frequency === 'weekly' ? '每週' : `每月 ${r.schedule.dayOfMonth} 號`} ${r.schedule.timeOfDay}`
            : r.type === 'site_trigger'
                ? `🌐 網站情境 (${r.rule.urlPatterns.length} 個網站)`
                : `首次開啟 (${r.schedule.cadence === 'daily' ? '每天' : '每週'})`;
    if (isSnoozed(r)) {
...
function renderList(ul, reminders) {
    ul.innerHTML = '';
    for (const r of reminders) {
...
async function handleAction(id, action, snoozeValue) {
    ...
    if (action === 'complete') {
        if (reminder.type === 'one_time')
            reminder.state.completedAt = now;
        else
            reminder.state.completedAt = now;
        ...
        if (reminder.type === 'one_time')
            reminder.state.snoozeUntil = snoozeUntil;
        else
            reminder.state.snoozeUntil = snoozeUntil;
        await updateReminder(reminder);
        ...
        if (reminder.type === 'one_time')
            reminder.state.snoozeUntil = null;
        else
            reminder.state.snoozeUntil = null;
        await updateReminder(reminder);
